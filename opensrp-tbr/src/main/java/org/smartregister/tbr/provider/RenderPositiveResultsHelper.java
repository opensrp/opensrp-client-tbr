...
public class RenderPositiveResultsHelper extends BaseRenderHelper {
    private static final String DETECTED = "detected";

    public RenderPositiveResultsHelper(Context context, ResultDetailsRepository detailsRepository) {
    ...
    public void renderView(String baseEntityId, View view, Map<String, String> extra) {
        ...
        if (extra.containsKey(LAST_INTERACTED_WITH) && !extra.get(LAST_INTERACTED_WITH).isEmpty()) {
            ...
            calendar.setTimeInMillis(Long.valueOf(extra.get(LAST_INTERACTED_WITH)));
            dateString += Constants.CHAR.SPACE + Utils.formatDate(calendar.getTime(), "dd MMM yyyy");
        }
