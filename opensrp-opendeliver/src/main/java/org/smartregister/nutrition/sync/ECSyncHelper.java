...
public class ECSyncHelper {
    ...
    public static ECSyncHelper getInstance(Context context) {
        if (instance == null) {
            instance = new ECSyncHelper(context, OpenDeliverApplication.getInstance().getEventClientRepository());
        }
        return instance;
    }
    ...
    public JSONObject fetchAsJsonObject(String filter, String filterValue) throws Exception {
        try {
            HTTPAgent httpAgent = OpenDeliverApplication.getInstance().getContext().getHttpAgent();
            String baseUrl = OpenDeliverApplication.getInstance().getContext().
