...
public class ResultsRepository extends BaseRepository {
    ...
    public JSONObject getLatestVisit(String baseEntityId, String event) {
        Cursor cursor = null;
        JSONObject json = null;
        try {
            cursor = getLatestVisitCursor(baseEntityId, event);
            if(cursor != null && cursor.moveToFirst()){
    ...
    private Cursor getLatestVisitCursor(String baseEntityId, String event) {
        Cursor cursor;
        SQLiteDatabase db = getReadableDatabase();
        String query =
                "SELECT * from event " +
                        " where baseEntityId = '" + baseEntityId + "' " +
                        (StringUtils.isNotBlank(event)?(" AND eventType = '"+event+"'"):"") +
                        " group by baseEntityId having max(updatedAt)";
        cursor = db.rawQuery(query, null);
        return cursor;
    }
    ...
    public String getLastVisitDate(String baseEntityId, String event) {
        Cursor cursor;
        SQLiteDatabase db = getReadableDatabase();
        String date = null;
        //TODO
        String query =
                "SELECT max(eventDate) as lastVisitDate from event " +
                        "where event.baseEntityId = '"+ baseEntityId +"' " +
                        (StringUtils.isNotBlank(event)?(" AND eventType = '"+event+"'"):"") +
