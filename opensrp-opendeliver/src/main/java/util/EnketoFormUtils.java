...
import static org.smartregister.nutrition.sync.TbrClientProcessor.CLIENT_EVENTS;
import static org.smartregister.nutrition.sync.TbrClientProcessor.CONTACT_SCREENING;
import static org.smartregister.nutrition.sync.TbrClientProcessor.DIAGNOSIS_EVENT;
import static org.smartregister.nutrition.sync.TbrClientProcessor.TREATMENT_INITIATION;
...
public class EnketoFormUtils {
    ...
    public EnketoFormUtils(Context context) {
        ...
        eventClientRepository = OpenDeliverApplication.getInstance().getEventClientRepository();
    }
    ...
    private Event tagSyncMetadata(Event event) {
        AllSharedPreferences sharedPreferences = OpenDeliverApplication.getInstance().getContext().userService().getAllSharedPreferences();
            ...
            try {
                ...
                if (event.getEventType() != null) {
                    if (event.getEventType().equals(DIAGNOSIS_EVENT)) {
                        ...
                        saveClient(client);
                    } else if (event.getEventType().equalsIgnoreCase(Constants.EVENT.NUTRITION_CLOSE_CASE) || event.getEventType().equals(Constants.EVENT.REMOVE_PATIENT) || event.getEventType().equals(Constants.EVENT.TREATMENT_OUTCOME)) {
                        JSONObject json = eventClientRepository.getClientByBaseEntityId(event.getBaseEntityId());
                        Client client = gson.fromJson(json.toString(), Client.class);
